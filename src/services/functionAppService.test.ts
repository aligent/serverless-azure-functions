...
describe("Function App Service", () => {
  ...
  const masterKeyUrl =
    "https://management.azure.comappId/host/default/listkeys?api-version=2019-08-01";
  const authKeyUrl = `${baseUrl}${app.id}/functions/admin/token?api-version=2016-08-01`;
  const syncTriggersUrl =
  ...
  const appSettingsUrl =
    `${baseUrl}/subscriptions/${subscriptionId}/resourceGroups/${config.provider.resourceGroup}` +
  ...
  beforeEach(() => {
    ...
    for (const funcName of Object.keys(functions)) {
      axiosMock
        .onDelete(
          `${baseUrl}${app.id}/functions/${funcName}?api-version=2016-08-01`
        )
    ...
    WebSiteManagementClient.prototype.webApps = {
      ...
      listApplicationSettings: jest.fn(() =>
        Promise.resolve({ properties: { ...appSettings } })
      ),
  ...
  it("get returns function app", async () => {
    ...
    expect(WebSiteManagementClient.prototype.webApps.get).toBeCalledWith(
      provider.resourceGroup,
      FunctionAppResource.getResourceName(slsService as any)
    );
  ...
  it("deletes function", async () => {
    const service = createService();
    const response = await service.deleteFunction(
      app,
  ...
  it("cleans up", async () => {
    ...
    for (let i = 0; i < functionNames.length; i++) {
      ...
      const call = logCalls.find(
        (c) => c[0] === `-> Deleting function: ${functionName}`
      );
  ...
  it("lists functions", async () => {
    const service = createService();
    expect(await service.listFunctions(app)).toEqual(
      functionsResponse.map((f) => f.properties)
    );
  });

  it("list functions with retry", async () => {
    axiosMock.onGet(listFunctionsUrl).reply(500);
    const service = createService();
  ...
  describe("Deployments", () => {
    ...
    beforeEach(() => {
      FunctionAppService.prototype.get = jest.fn(() =>
        Promise.resolve(expectedSite)
      );
      (FunctionAppService.prototype as any).sendFile = jest.fn();
      ArmService.prototype.createDeploymentFromConfig = jest.fn(() =>
        Promise.resolve(expectedDeployment)
      );
      ArmService.prototype.createDeploymentFromType = jest.fn(() =>
        Promise.resolve(expectedDeployment)
      );
      ArmService.prototype.deployTemplate = jest.fn(() =>
    ...
    it("deploys ARM templates with custom configuration", async () => {
      ...
      expect(ArmService.prototype.createDeploymentFromConfig).toBeCalledWith(
        slsService.provider["armTemplate"]
      );
    ...
    it("deploys ARM template from well-known (default) configuration", async () => {
      ...
      expect(ArmService.prototype.createDeploymentFromType).toBeCalledWith(
        ArmTemplateType.Consumption
      );
      expect(ArmService.prototype.deployTemplate).toBeCalledWith(
        expectedDeployment
      );
  ...
  it("publishes functions and uploads to blob storage with default naming convention", async () => {
    ...
    const defaultArtifact = path.join(
      ".serverless",
      `${sls.service.getServiceName()}.zip`
    );
    ...
    expect((FunctionAppService.prototype as any).sendFile).toBeCalledWith(
      {
        ...
        headers: {
          Authorization: `Bearer ${
            (await variables["azureCredentials"].getToken()).accessToken
          }`,
          Accept: "*/*",
    ...
    const expectedArtifactName = service
      .getDeploymentName()
      .replace("rg-deployment", "artifact");
    expect(
      (AzureBlobStorageService.prototype as any).uploadFile
    ).toBeCalledWith(
      defaultArtifact,
      constants.deploymentConfig.container,
      `${expectedArtifactName}.zip`
    );
    const uploadCall = (AzureBlobStorageService.prototype as any).uploadFile
      .mock.calls[0];
  ...
  it("uses default name when no artifact provided", async () => {
    ...
    delete sls.service["artifact"];
    const service = createService(sls);
    expect(service.getFunctionZipFile()).toEqual(
      path.join(".serverless", `${sls.service.getServiceName()}.zip`)
    );
  });
  ...
  it("adds a new function app setting", async () => {
    ...
    const sendApiSpy = jest.spyOn(
      FunctionAppService.prototype as any,
      "sendApiRequest"
    );
    ...
    expect(sendApiSpy).toBeCalledWith("PUT", appSettingsUrl, {
      data: {
  ...
  describe("Updating Function App Settings", () => {
    ...
    afterEach(() => {
      (
        FunctionAppService.prototype.updateFunctionAppSetting as any
      ).mockRestore();
    });
    ...
    it("updates WEBSITE_RUN_FROM_PACKAGE with SAS URL if configured to run from blob", async () => {
      ...
      await service.uploadFunctions(app);
      expect(
        AzureBlobStorageService.prototype.generateBlobSasTokenUrl
      ).toBeCalled();
      expect(
        FunctionAppService.prototype.updateFunctionAppSetting
      ).toBeCalledWith(app, "WEBSITE_RUN_FROM_PACKAGE", sasUrl);
      expectLogFunctions(sls);
    });
    ...
    it("does not upload directly to function app if configured to run from blob", async () => {
      ...
      FunctionAppService.prototype.uploadZippedArtifactToFunctionApp =
        jest.fn();
      await service.uploadFunctions(app);
      expect(
        AzureBlobStorageService.prototype.generateBlobSasTokenUrl
      ...
      (
        FunctionAppService.prototype.uploadZippedArtifactToFunctionApp as any
      ).mockRestore();
    });
  ...
  function expectLogFunctions(sls: Serverless) {
    ...
    expectSlsLogContains(
      sls,
      "-> hello: [*] myHostName.azurewebsites.net/api/hello"
    );
    expectSlsLogContains(
      sls,
      "-> goodbye: [*] myHostName.azurewebsites.net/api/goodbye"
    );
  }
